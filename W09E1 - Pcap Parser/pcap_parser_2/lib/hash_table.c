...
void insert_to_flow(Node *const pkt_node, enum InsertAlgorihm insert_type,
  ...
  if (insert_type == DESC) {
    insert_node_desc(flow_direction, pkt_node, fptr);
  }
...
flow_base_t *search_flow(HashTable const table, uint64_t key, FILE* fptr) {
  ...
  if (!head_flow) {
    ...
    if (DEBUGGING) fprintf(fptr, "Finding node with index = %d...\n", index);
    Node *n = search_node(head_flow, key);
    if (DEBUGGING) fprintf(fptr, "Done searching node with index = %d...\n", index);
...
parsed_payload pop_head_payload(Node **flow_direction) {
  ...
  if (flow_direction) {
    ...
    if (node == NULL) {
      fprintf(fptr, "flow is empty, nothing to delete\n");
