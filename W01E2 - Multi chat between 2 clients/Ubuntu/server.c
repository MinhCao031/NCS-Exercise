...
void * doNetworking(void * ClientDetail){
	...
	while(1){
		...
		if(strcmp(data,"LIST") == 0){
			int l = 0;
			for(int i = 0 ; i < clientCount ; i ++) if(i != index) {
...
int main(){
	...
	while(1){
		Client[clientCount].sockID = accept(
			serverSocket,
		...
		pthread_create(
			&thread[clientCount], NULL,
			doNetworking, (void *) &Client[clientCount]);
		clientCount ++;
		pthread_join(thread[clientCount],NULL);
	}
